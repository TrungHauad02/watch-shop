...
export default function BrandsPage() {
  ...
  const fetchBrands = async (
    ...
    try {
      ...
      const response = await brandsService.getBrands(page, pageSize, filter);
      ...
      if (response.status) {
        ...
        setError(
          response.message || 'Có lỗi xảy ra khi tải danh sách thương hiệu'
        );
        setBrands([]);
        setPagination({
          currentPage: 0,
  ...
  // Initial load
  useEffect(() => {
    fetchBrands(initialPage, initialPageSize, initialSearchQuery);
  }, []);
  ...
  const handleSearch = () => {
    ...
    fetchBrands(newPage, pagination.pageSize, searchQuery, true);
  };
  ...
  const handleSearchChange = (value: string) => {
    ...
    // If search is cleared, fetch all brands immediately
    if (!value.trim()) {
      const newPage = 0;
      updateURL('', newPage, pagination.pageSize);
      fetchBrands(newPage, pagination.pageSize, '', true);
    }
  };
  ...
  const handlePageChange = (page: number) => {
    ...
    fetchBrands(page, pagination.pageSize, searchQuery);
  ...
  const handlePageSizeChange = (pageSize: number) => {
    ...
    fetchBrands(newPage, pageSize, searchQuery);
  ...
  return (
    <WSDecorativeBackground
      ...
      density="low"
      sx={{
        background: `linear-gradient(-135deg, ${COLORS.gray600} 0%, ${COLORS.gray300} 50%, ${COLORS.gray50} 100%)`,
      }}
    >
      <Container maxWidth="xl" sx={{ py: 4 }}>
        ...
        <Box sx={{ mb: 4 }}>
          ...
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            ...
            >
              Khám phá những thương hiệu đồng hồ danh tiếng hàng đầu thế giới
              với chất lượng và thiết kế vượt trội
            </Typography>
          </Box>
            ...
            <Fade in={!searchLoading}>
              <Box sx={{ textAlign: 'center', mb: 3 }}>
                ...
                >
                  {hasResults ? (
                    <>
                      Tìm thấy <strong>{pagination.totalElements}</strong>{' '}
                      thương hiệu cho từ khóa <strong>"{searchQuery}"</strong>
                    </>
                  ) : !loading && !searchLoading ? (
                    <>
                      Không tìm thấy thương hiệu nào cho từ khóa{' '}
        ...
        {/* Brands Grid */}
        <Box sx={{ mb: 4 }}>
          <BrandsGrid brands={brands} loading={loading || searchLoading} />
        </Box>
